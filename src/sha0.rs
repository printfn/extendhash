...
/// # Returns
///
/// This function returns SHA-0 padding for the given input size. This
...
/// Compute the SHA-0 padding length (in bytes) for the given input length.
///
...
/// # use extendhash::sha0;
/// let data = "This string will be hashed.";
/// let padding_length = sha0::padding_length_for_input_length(data.len());
...
mod tests {
	...
	fn slightly_longer_test() {
		...
		assert_eq!(sha0::compute_hash(input.as_bytes()), [
			0xd2, 0x51, 0x6e, 0xe1, 0xac, 0xfa, 0x5b, 0xaf, 0x33, 0xdf,
			0xc1, 0xc4, 0x71, 0xe4, 0x38, 0x44, 0x9e, 0xf1, 0x34, 0xc8]);
	}
	...
	fn test_hash_ext() {
		...
		let intermediate_padding = sha0::padding_for_length(secret.len());
