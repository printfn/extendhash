...
/// # Returns
///
/// This function returns SHA-1 padding for the given input size. This
...
/// # use extendhash::sha1;
/// let data = "This string will be hashed.";
/// let padding_length = sha1::padding_length_for_input_length(data.len());
...
/// # Returns
///
/// This function returns the SHA-1 hash of the concatenation of the original
...
mod tests {
	...
	#[test]
	fn a_test() {
		assert_eq!(sha1::compute_hash("a".as_bytes()), [
	...
	fn quick_brown_fox_test() {
		assert_eq!(sha1::compute_hash("The quick brown fox jumps over the lazy dog".as_bytes()), [
	...
	fn quick_brown_fox_test_2() {
		assert_eq!(sha1::compute_hash("The quick brown fox jumps over the lazy cog".as_bytes()), [
	...
	fn long_test() {
		...
		assert_eq!(sha1::compute_hash(input.as_bytes()), [
	...
	fn test_hash_ext() {
		...
		let intermediate_padding = sha1::padding_for_length(secret.len());
